...
import Nimma from 'https://unpkg.com/nimma@0.2.0/dist/esm/index.mjs';

async function generateQuery(expressions) {
  const __dirname = path.dirname(fileURLToPath(import.meta.url));
  const nimma = new Nimma(expressions);

  const filepath = path.join(__dirname, '../.gen/nimma-0.2.2.mjs');
  await fs.writeFile(
    filepath,
    nimma.sourceCode.replace(
      /nimma\/runtime/g,
      'https://unpkg.com/nimma@0.2.2/dist/esm/runtime/index.mjs',
    ),
  );
...
export default async (suite, document, expressions) => {
  ...
  suite.add(
    `Nimma@0.2.2 (no precompiled code)`,
